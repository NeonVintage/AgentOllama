...
echo ========================================
echo OllamaAgent GUI Build Script
echo ========================================
echo.
...
if not exist "%CURL_INCLUDE%\curl\curl.h" (
    echo Installing curl via vcpkg...
...
cl /nologo /EHsc /std:c++17 /O2 ^
    ...
    build\app.res ^
    /Fe:build\ollama_agent_gui.exe ^
    /Fo:build\ ^
    /link /SUBSYSTEM:WINDOWS /LIBPATH:"%CURL_LIB%" libcurl.lib ws2_32.lib user32.lib gdi32.lib comctl32.lib shell32.lib ole32.lib
...
echo ========================================
echo.
echo GUI Executable: build\ollama_agent_gui.exe
echo.
echo To run, double-click ollama_agent_gui.exe
